  ...
  protected function parse_response($response, $format = NULL) {
    ...
    switch ($format) {
      case 'json':
        ...
        } elseif(is_array($response_decoded)) { //imiller wasn't always an object
          // if we're getting an array of objects back as JSON
          foreach ($response_decoded as &$item) {
            $item['id'] = $item['id_str'];
          }
        } else {
          watchdog('twitter', 'Twitter returned unexpected results: '.$response_decoded);
        }
