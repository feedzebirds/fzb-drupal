 ...
 * Author: Ira Miller
 * 
 * Utility functions for managing communication with the CyPay server.
 */
require_once("cypay_config.php");
require_once("cypay_error.php");

/**
 * Send request to CyPay.
 *
 * Parameters:
 * params           An array of parameters to send to CyPay as POST data
...
function sendToGateway($params, $gate, $key = "", $secret = "") {
  ...
  if(empty($key))
    return new cyError(20, "Invalid credentials");

  $params->key = $key;

  //add HMAC
  $hmac = serialHMACEncode($params, $secret);
  if(!$hmac) return new cyError(1, "Error hashing data for post");
  $params->hmac = $hmac;
  ...
  $resposne = "";
  if(curl_errno($curl))
    $response = new cyError(2, "cURL error: ".curl_errno($curl));
  curl_close($curl);

  //if response is an error, return it
  if($response && get_class($response) == "cyError") return $response;

  //validate response and check for server errors
  $response = parseResponse($rawresponse, $secret);
  if(!$response)
    return new cyError(3, "Malformed response");

  return $response;
}

/**
 * Generate HMAC by json encoding parameter array then running through SHA256.
 *
 * Parameters:
...
function serialHMACEncode($params, $secret = "") {
  ...
  //generate hmac
  $hmac = hash_hmac("sha256", $jMessage, $secret);
  return $hmac;
}
 ...
 *
 * Parameters:
 * raw              The raw response from Cypay gateway
 * secret           Your CyPay application secret
 *
